...
public class UpdateVehiclePriceCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        try {
            ...
            vehicleService.updateVehiclePrice(form);
    ...
    public DefaultForm doForm(HttpServletRequest request) {
        ...
        if (vehicleId == null || vehicleId.isBlank()) {
            throw new IllegalArgumentException("no parameter 'vehicleId'");
        }

        String newPrice = request.getParameter(VEHICLE_NEW_PRICE_PARAM);
        if (newPrice == null) {
            throw new IllegalArgumentException("no parameter 'price'");
        }

        try {
            form.setVehicleId(Long.parseLong(vehicleId));
            form.setPrice(Integer.parseInt(newPrice));
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Error parsing argument(s) from String");
        }
