...
CREATE TABLE `message` (
  ...
  `topic` varchar(45) COLLATE utf8mb4_general_ci NOT NULL,
  `message` varchar(200) COLLATE utf8mb4_general_ci NOT NULL,
  `date_sent` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `date_read` datetime DEFAULT NULL,
  PRIMARY KEY (`message_id`),
  UNIQUE KEY `id_UNIQUE` (`message_id`),
  KEY `sender_id_idx` (`sender_id`),
  KEY `receiver_id_idx` (`receiver_id`),
  CONSTRAINT `reciever_id` FOREIGN KEY (`receiver_id`) REFERENCES `user` (`user_id`),
...
-- Dump completed on 2021-08-24 16:23:46
